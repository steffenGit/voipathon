...
window.addEventListener('load', function() {
  ...
  document
    .getElementById('usernameOkButton')
    .addEventListener('click', function() {
      let textFieldValue = document.getElementById('usernameTF').value;
      if (textFieldValue !== '' && textFieldValue !== null && !isUsernameSet) {
        document
          .getElementById('userInfoDiv')
          .appendChild(document.createTextNode('Username: ' + textFieldValue));
        isUsernameSet = true;
...
function addGroup(group) {
  addToList(document.getElementById('groupList'), group, function() {
    ...
    //todo attach group
    //get group number;
    const groupNumber = parseInt(group.substr(group.length - 1));
    fsm.attachGroup(groupNumber);
  });
}
